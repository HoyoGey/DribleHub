...
local SectionPreset = game:GetObjects("rbxassetid://7121846230")[1] -- from solaris :)
...
function lib:Window(text, preset, closebind)
    ...
    task.wait(0.3)
    HiText.Text = "Thanks for using Drible Hub"
    task.wait(0.1)
    TweenService:Create(HiText, TweenInfo.new(0.3), { TextTransparency = 0 }):Play()
    wait(1)
    TweenService:Create(HiText, TweenInfo.new(0.3), { TextTransparency = 1 }):Play()
        ...
        function Sectt:Section(text)
            ...
            function tabcontent:Slider(text, min, max, start, inc, callback)
                ...
                local function move(Input)
                    local XSize = math.clamp((Input.Position.X - SlideFrame.AbsolutePosition.X) /
                        SlideFrame.AbsoluteSize.X,
                        0, 1)
